...
|---|---|---|---|---|---|---|
| TL-001 | Linear | Work Mgmt | Authoritative source for tasks/sprints | Gemini | ACTIVE | APPROVED |
| TL-002 | GitHub | Source | Authoritative source for code/PRs | Andrea | ACTIVE | APPROVED |
| TL-003 | GitHub Actions | CI/CD | Pipeline: Lint, Format Check, TypeCheck, Test, Build, Scan | Gemini | APPROVED | APPROVED |
| TL-004 | Dependabot | SecOps | Auto update deps (PR only, no auto-merge) | Gemini | APPROVED | APPROVED |
| TL-005 | GitHub CodeQL | SecOps | Static security analysis (Subject to repo capabilities) | Gemini | APPROVED | APPROVED |
| TL-006 | Secret Scanning | SecOps | Block leak of keys/tokens (Native + CI Fallback) | Gemini | ACTIVE | APPROVED |
| TL-007 | Supabase CLI | Backend | Local CLI scaffold only | Codex | APPROVED | APPROVED (Local M-001) |
| TL-008 | Vercel | Hosting | PR Preview generation (Required after integration) | Gemini | APPROVED | APPROVED |
| TL-009 | Motion | Frontend | UI Animation | Claude | EVALUATION | PENDING EVAL |
